...
# 1. FastAPI 인스턴스 생성: 애플리케이션 객체 생성 및 메타데이터(제목, 버전 등) 설정(app 함수 )
# 2. CORS 및 미들웨어 설정: 도메인 간 요청 허용, 요청/응답 로깅, 인증 미들웨어 등(도메인 연결)
# 3. 생명주기 이벤트: 애플리케이션 시작시 DB 테이블 생성/연결, 종료시 DB 연결해제
# 4. 라우터(Router) 등록: api/ 디렉토리에 분리해 둔 엔드포인트들을 앱에 연결
# 5. 전역 예외 처리기 (Exception Handler): 앱 전체에서 발생하는 공통 에러 핸들링(로그인)
...
# 파이썬에서 @ 기호는 "이 함수에 특별한 기능을 덧붙이겠다"라는 표시입니다.(역할을 하는 표시자)
# 이를 데코레이터(Decorator)라고 부릅니다.(백엔에서 프론트에 연결하여 등록하는 함수역할)
